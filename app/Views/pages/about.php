<section class="max-w-6xl mx-auto px-4 py-16">
    <h1 class="text-4xl font-semibold">Despre noi</h1>
    <p class="mt-6 text-zinc-300 max-w-3xl">
        Secret Doors produce uși filomuro și sisteme uși invizibile la comandă, pentru design interior modern.
        Punem accent pe uși ascunse în perete, uși invizibile cu balamale ascunse și toc ascuns aluminiu, potrivite atât pentru apartamente, cât și pentru birouri în București / România și Moldova (Chișinău).
    </p>

    <div class="grid md:grid-cols-3 gap-6 mt-10">
        <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <h3 class="font-semibold">Avantaje competitive</h3>
            <p class="text-zinc-400 mt-2">Producție locală, uși premium interior la comandă, montaj uși filomuro asistat.</p>
        </div>
        <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <h3 class="font-semibold">Proces de producție</h3>
            <p class="text-zinc-400 mt-2">Consultanță, proiectare, fabricație, livrare și instalare plintă ascunsă / profile LED perete.</p>
        </div>
        <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <h3 class="font-semibold">Calitate premium</h3>
            <p class="text-zinc-400 mt-2">Materiale verificate, uși filomuro fără pervaz și finisaje MDF, cornișă ascunsă.</p>
        </div>
    </div>

    <div class="mt-12 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8">
        <div class="grid md:grid-cols-2 gap-10 items-start">
            <div>
                <h2 class="text-2xl font-semibold mb-4">Lucrările noastre</h2>
                <p class="text-zinc-300 leading-7">
                    Uși ascunse pentru apartament și birouri, uși invizibile personalizate și uși filomuro vopsite.
                    Oferim soluții constructive pereți și elemente conexe: plintă filomuro, plintă invizibilă, profile aluminiu interior și cornișă iluminare indirectă.
                </p>
            </div>
            <div class="flex justify-center md:justify-end">
                <blockquote class="tiktok-embed" cite="https://www.tiktok.com/@secret.doors.prem/video/7613373599239671060" data-video-id="7613373599239671060" style="max-width: 605px;min-width: 325px;">
                    <section>
                        <a target="_blank" title="@secret.doors.prem" href="https://www.tiktok.com/@secret.doors.prem?refer=embed">@secret.doors.prem</a>
                        <p>Uși FiloMuro direct de la producător</p>
                        <a target="_blank" title="♬ sunet original - Secret Doors Premium" href="https://www.tiktok.com/music/sunet-original-7613373639093668628?refer=embed">♬ sunet original - Secret Doors Premium</a>
                    </section>
                </blockquote>
                <script async src="https://www.tiktok.com/embed.js"></script>
            </div>
        </div>
    </div>
</section>
